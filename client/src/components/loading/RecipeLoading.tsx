...
export default function RecipeLoading({
  ...
  /** Streamed in live from /generate/stream as the model writes the recipe. */
          ...
          {partialTitle ? (
            ...
            >
              <p className="text-[15px] font-medium text-white/80">
                Writing <span className="text-accent">{partialTitle}</span>…
              </p>
              {partialDescription && (
                <p className="mx-auto mt-2 max-w-md text-sm text-white/45">{partialDescription}</p>
              )}
            </motion.div>
          ) : (
            <motion.p
