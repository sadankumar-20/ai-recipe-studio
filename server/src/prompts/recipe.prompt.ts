...
export function buildRefinePrompt(instruction: string, currentRecipe: unknown): string {
  return `Here is an existing recipe as JSON:
${JSON.stringify(currentRecipe)}

The user wants exactly one refinement applied to it: "${instruction}".

Apply ONLY that change. Keep every other field byte-for-byte identical where possible —
same title (unless the change renames the dish), same step ids and order, same tips,
same servings. Only adjust fields the change genuinely requires (e.g. update
"ingredients" and "nutrition" if an ingredient is added/removed/substituted, or
"steps" if the technique changes). Do not invent unrelated edits.
Return the FULL updated recipe as a single JSON object following the required schema exactly.`;
}
