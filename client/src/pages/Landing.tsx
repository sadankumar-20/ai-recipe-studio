...
// ============================================================================
// Landing — the front door. Two paths into the same generation pipeline:
// free-form ingredient chips (handleGenerate) or a curated dish picked from
// the cuisine explorer (handleSelectDish). Both stream the response so the
// loading line can show the recipe title while it's still being written,
// then navigate to /workspace once the validated recipe is in the store.
// ============================================================================
...
export default function Landing() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const recipe = await generateRecipeStreaming({ ingredients }, setStreamPreview);
      setRecipe(recipe);
  ...
  const handleSelectDish = async (params: {
    ...
    try {
      const recipe = await generateRecipeStreaming(
        { dishName: params.dishName, cuisineHint: params.cuisineLabel },
        setStreamPreview
      );
      setRecipe(recipe, {
        ...
        <section className="mx-auto -mt-4 max-w-2xl px-4 sm:px-6">
          ...
          {isLoading && (
            ...
            >
              {streamPreview?.title ? (
                <>
                  Writing <span className="text-accent">{streamPreview.title}</span>...
                </>
              ) : (
